...
**CLI root.** The commands below use direct tree access — the scan/analysis/plot tools live in the `spec-file` tree, action-log queries in `db`. That is exactly the form the planner uses (`beamtimehero spec-file …`, `beamtimehero db …`). If your role is a scoped branch instead (the data-collection agent runs everything under `collector`), insert your branch after `beamtimehero`: e.g. `beamtimehero collector spec-file analyze-efficiency …`. Use whichever form your role permits — the tree name and command are the same either way.
...
2. `beamtimehero spec-file list-scans --limit 20` — confirm the actual scan count matches `reps_completed`. The data-collection agent occasionally throws scans out via a SPEC note; the file count is authoritative.
3. **Identify a feature, then read its bounds off the plot.** This is the agent's responsibility — there is no auto-windowing.
    - `beamtimehero spec-file average-scans --file-name <sample_id>` to see the averaged spectrum first, OR `plot-scan-stack --file-name <sample_id>` for a stacked-reps view. Read off the energy bounds of the feature you care about (white-line peak, pre-edge peak, dip between oscillations 1 and 2, etc.) — pick a tight window that brackets the feature with a small margin.
...
4. `beamtimehero spec-file analyze-efficiency --file-name <sample_id> --e-min <e_min> --e-max <e_max>` — **primary verdict**, run on the windowed feature. `e_min`/`e_max` are required. Returns `verdict` ∈ {`needs_more`, `reasonable`, `marginal`, `wasteful`} plus the four keys that carry the decision:
    - `cumulative_sem_pct[]` — fractional SEM of the merge of the first *n* reps. `None` at n=1 (one scan has no scatter estimate). This is the convergence curve.
    - `cumulative_floor_pct[]` — the counting-statistics floor at each rep count, from the counts actually recorded. The curve cannot go below it. Ratio ~1 = photon-limited, more reps help as 1/√n; ratio rising = systematics.
    - `target_reached_at_rep` / `reps_to_target` — the rep at which the SEM met the target, or how many reps in total it would take at this count rate. `reps_to_target` is the number to reason about against remaining beamtime: if it doesn't fit, the levers are count time, filters, or a looser target — **not** more reps of the same length.
    - `plateau_from_rep` / `limited_by` — the rep at which the curve left the floor, and whether the merge is currently capped by `counting_statistics` or `systematics`. A plateau means further reps are wasted and something needs fixing; the reps *before* it are the clean data.

    Also returns `cv_mean_pct`, `counts_poisson_floor_pct`, `cv_vs_floor_ratio`, `optimal_scan_count`, `marginal_improvement[]`, and the full `convergence` sub-result. `cumulative_cv_pct[]` is a deprecated alias of `cumulative_sem_pct[]` — same numbers, wrong name (it is a standard error, not a coefficient of variation).
5. `beamtimehero spec-file analyze-feature-evolution --file-name <sample_id> --e-min <e_min> --e-max <e_max> --statistic <stat>` — **stationarity check on the published number.** Reduces each rep to a single scalar over the window (white-line height with `max`; white-line position with `argmax`; peak area with `integral`; height-above-baseline with `height`) and reports the per-rep trace, running mean, running SEM, and a verdict ∈ {`converged`, `marginal`, `needs_more`, `drifting`}. The default target is `sem_threshold_frac=0.01` (1% of mean); tighten to 0.005 for a small feature driving a result.
    - **`drifting` is the verdict to act on.** It fires on `is_drifting` (a Mann-Kendall monotone trend over the whole trace, material in size) or `sem_is_rising` (the running SEM grew — repeats of one measurement cannot do that). Either means you are averaging a moving target, so the SEM is getting more confident about the wrong number. Investigate beam damage, spot drift or energy calibration; do not simply collect more.
    - **Do not use this as the primary stop signal.** It collapses the spectrum to one number, and it falls as 1/√n for as long as the reps are repeats — so it eventually crosses any threshold, on any sample, including one being destroyed. It tells you how precisely you know one scalar; `analyze-efficiency` tells you whether the *spectrum* has converged and whether more reps can still help.
    - Prefer `--statistic integral` over `max` for a white line. `max` is an order statistic: it is upward-biased and it deliberately seeks the noisiest point in the window, which inflates rep-to-rep scatter for no gain. `integral` uses every point in the window.
6. `beamtimehero spec-file analyze-convergence --file-name <sample_id> --e-min <e_min> --e-max <e_max>` — secondary read (cosine similarity). Useful for spotting outliers in `individual_vs_mean` (flag any < 0.95). The cumulative-convergence number is amplitude-dominated even windowed; treat as background context, not as a stop signal.
7. `beamtimehero spec-file group-scans-by-spot --file-name <sample_id>` — list spot clusters by Sx/Sy/Sz. If `n_spots > 1`, run the per-spot variant next.
8. `beamtimehero spec-file analyze-per-spot --file-name <sample_id> --e-min <e_min> --e-max <e_max>` — convergence per spot plus a between-spot vs within-spot heterogeneity F-statistic. F~1: spots agree, safe to combine. F>>1 (>5): spots disagree beyond shot noise — the combined average is a population mean, not a single chemistry; more reps will not converge.
9. `beamtimehero spec-file average-scans --file-name <sample_id> --e-min <e_min> --e-max <e_max> [--weighting inverse_variance]` — get the running mean and std for a sanity-check overlay. Use `inverse_variance` weighting when reps came from spots with very different signal levels and you want SNR-optimal averaging across them.
...
3. **Run `analyze-efficiency` with `--e-min` and `--e-max` for the chosen feature window.** Capture: `verdict`, `limited_by`, `target_reached_at_rep`, `reps_to_target`, `plateau_from_rep`, `cumulative_sem_pct[]`, `cumulative_floor_pct[]`, `cv_vs_floor_ratio`. Read them in that order — `limited_by` says whether more reps can help at all, `target_reached_at_rep` / `reps_to_target` says how much more is needed, and `plateau_from_rep` says where the good data stops if a systematic entered mid-series.
4. **Run `analyze-feature-evolution` for the chosen feature.** Pass the same `--e-min` / `--e-max` plus a statistic that captures what you actually care about:
    ...
    - Returns a per-rep scalar trace, its running mean, running SEM, and a verdict in {`converged`, `marginal`, `needs_more`, `drifting`}. The bar is `final_sem_frac < 0.01` (1% of the mean) AND not drifting — `is_drifting` false and `sem_is_rising` false. Tighten the SEM threshold to 0.005 for a small feature driving a result. `final_drift_frac` is still reported but is no longer the drift gate: it is the step in the *running* mean, so its sensitivity dies as 1/n and at 30 reps a rep 20% off the mean slips under 1%.
5. **Visually inspect the accumulating stack (mandatory, every invocation).** The numbers above are necessary but not sufficient. Always look at the plot:
    - `plot-scan-stack --file-name <id> --e-min <e_min> --e-max <e_max>` — overlays all reps on the feature window, color-progressed by rep order. Look for: reps scattering symmetrically around a stable mean (converged), reps drifting monotonically in one direction (the sample is evolving — go to `assess-sample-damage`, do not "extend reps to resolve it"; a monotone trend does not average out), or reps falling into discrete bands (between-spot heterogeneity, see step 6).
...
6. **If multiple spots are present, run the per-spot analysis.** Call `group-scans-by-spot --file-name <id>` first. If `n_spots > 1`, also call `analyze-per-spot --file-name <id> --e-min <e_min> --e-max <e_max>`:
    ...
    - **What to surface.** If `analyze-per-spot` reports `heterogeneous`, do not mark the sample `done` on the strength of a `reasonable` combined-stack (all-spots) verdict. Surface to staff via `post-status-update`: the question becomes whether the experiment wants a population average (continue with the unweighted mean) or per-spot spectra (split the file and analyze each subset; sample stays open).
...
The primary lever is the **`analyze-efficiency` verdict on the merge**, read through `limited_by`: it is the only signal that distinguishes "more reps will help" from "more reps are wasted". The secondary lever is the **`analyze-feature-evolution` verdict** on the chosen feature, which guards the published number against drift. The tertiary signal is the visual half-vs-half and stacked-reps plots. (All of these are windowed — there is no whole-spectrum verdict.)

Read them in this order:

1. **`limited_by == "systematics"`, or feature verdict `drifting`** — stop and diagnose. Neither is fixed by collecting more.
2. **`target_reached_at_rep` is set** — you have the statistics you asked for. Stop, trim the remaining reps, and move the budget.
3. **Neither** — you are photon-limited and still short. `reps_to_target` says how much more it costs; decide against the queue ahead.

| Primary: efficiency | Feature verdict | Recommended action |
|---|---|---|
| `limited_by: systematics` (`plateau_from_rep` set) | any | **Stop collecting on this sample.** The merge left the counting-statistics floor at `plateau_from_rep` — the reps from there on carry a systematic that averaging will not remove, and the reps before it are your clean data. Surface via `post-status-update` with the plateau rep. Probable causes in order: a spot change or sample damage (run `assess-sample-damage`), between-spot heterogeneity (`analyze-per-spot`), detector saturation, drifting energy calibration. Do not extend `reps`. |
| any | `drifting` | **Stop and diagnose**, same as above, whatever the SEM level. `is_drifting` or `sem_is_rising` means the feature is moving, so more reps buy precision on a moving target. Run `assess-sample-damage` and check `record-observable-trend`. |
| `target_reached_at_rep` set, verdict `wasteful` | `converged` or `marginal` | **Stop now.** Trim via `set-sample-time-budget --reps <reps_completed>`. Roll the freed budget into samples flagged `needs_more`. The explanation states how many reps went past the target. |
| `target_reached_at_rep` set, verdict `reasonable` | `converged` | Sample is publication-quality. Let `reps_completed` reach the planner target if it is close, then move on. If multiple features are named, only stop when all are `converged`. |
| `target_reached_at_rep` set | `needs_more` | The spectrum met its target but the chosen scalar has not. Usually the window or statistic is wrong for the feature — check you are not using `--statistic max` on a narrow window. If the window is right, this is a genuinely demanding feature: extend if the budget allows, else document the shortfall. |
| `needs_more`, `limited_by: counting_statistics` | any | **Keep collecting — reps still buy SNR as 1/√n.** Compare `reps_to_target` against remaining beamtime. If it does not fit, do not just collect more of the same: raise count time, remove a filter, or accept a looser target, and say which in the `record-sample-progress --note`. |
| `cv_vs_floor_ratio` < 1 | any | Suspect — likely a wrong active counter or a stack with mixed counters. Re-check `get-active-counter` and the file's column set before trusting anything. (Sitting *below* the counts floor is not physically possible, so it means the floor and the curve are describing different data.) |
| outlier present | any | `analyze-convergence.individual_vs_mean < 0.95`. Surface via `post-status-update`; staff or data-collection may throw a scan out via SPEC note before the next decision. A single bad rep also shows up as a `plateau_from_rep` at that rep — the fix there is to drop the rep and re-merge, not to stop the sample. |
...
- `analyze-feature-evolution` reports `needs_more` for any named feature **and** `limited_by` is `counting_statistics`. The feature bar still has to be met, but only more photons can meet it — if `limited_by` is `systematics` or the feature verdict is `drifting`, collecting more is the wrong move and this override does not apply.
- `plot-first-half-vs-second-half` reports max |Δ|/SEM > 3σ inside the feature window — halves disagree. Note this is *also* the signature of drift, so check the feature verdict before reading it as "not yet stationary": if the trace is `drifting`, halves will keep disagreeing however long you collect.
...
- **CV and cosine similarity are amplitude-dominated — this is why the window is mandatory and `analyze-feature-evolution` is primary.** A statistic taken over the whole spectrum is dominated by the post-edge plateau (defined ~1.0 by edge-step normalization) and would report convergence while a small feature is still resolving — which is exactly why every convergence tool requires a feature window (there is no whole-spectrum mode). Even inside a tight window the cumulative cosine number stays amplitude-weighted, so treat `analyze-convergence` as background and let `analyze-feature-evolution` plus the visual inspections (steps 4–5) drive the stop decision. See "Why the tools require a feature window" at the top of this skill.
