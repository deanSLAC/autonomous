...
The primary lever is the **`analyze-feature-evolution` verdict on the chosen feature window** (`converged` / `marginal` / `needs_more`). The secondary lever is the **`analyze-efficiency` verdict** plus `cv_vs_floor_ratio`. The tertiary signal is the visual half-vs-half and stacked-reps plots. (All of these are windowed — there is no whole-spectrum verdict.)
