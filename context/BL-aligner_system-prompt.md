...
- **`plotselect` BEFORE the scan.** `peak`/`cen` operate on whichever
  ...
  `post-scan-move`.
- **Plot every scan and read the PNG before deciding peak vs cen.**
  Run `tool plot-scan` (or equivalent) and read the image. The curve
  shape — sharp peak, broad plateau, asymmetric, double-humped, noisy
  — determines which post-scan-move is appropriate. Do not pick peak
  vs cen from the shortcut name alone.
